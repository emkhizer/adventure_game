...
class Character {
    name; // Character ka name store karne ke liye variable
    fuel = 100; // Character ka initial fuel level 100 set kiya gaya hai
    constructor(name) {
...
} // Player class jo Character se inherit karti hai
class Opponent extends Character {
} // Opponent class jo Character se inherit karti hai
...
while (true) {
    ...
    if (action === "Attack") {
        ...
        if (num > 0) {
            ...
            console.log(chalk.bold.green(`${opponent.name} fuel is ${opponent.fuel}`)); // Opponent ka fuel console mein display hota hai
            if (player.fuel <= 0) {
        ...
        console.log(chalk.bold.italic.green(`You drink health potion. Your fuel is ${player.fuel}`)); // Message display hota hai ke player ne health potion pee liya
    }
